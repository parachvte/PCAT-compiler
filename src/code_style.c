...
#define next_offset    (offset + 3)
#define gopi(k)        _print_ast_code_style(pick_ast(x,k), next_offset);
#define gop(k)         _print_ast_code_style(pick_ast(x,k), offset);
...
#define p              printf
#define SEPLIST(sep)   do { i=0; FOREACH{ if ( i > 0 ){p(sep);p(" ");}go(ELEM); i += 1; } } while (0)
#define record_line    do { x->line_no = line_no; } while(0)
